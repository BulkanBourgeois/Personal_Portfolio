...
Command: npx @threlte/gltf@3.0.1 static/models/room.glb -o src/lib/scene/models/RoomModel.svelte -t -k --root static
-->
...
<script lang="ts">
  ...
  let {
    ...
    onTvClick?: () => void
    onTvHoverChange?: (isHovering: boolean) => void
  } = $props()
...
>
  ...
  {:then gltf}
    ...
    <T.Mesh
      ...
      scale={[4.13, 2.14, 3.5]}
      onclick={() => onTvClick?.()}
      onpointerenter={() => onTvHoverChange?.(true)}
      onpointerleave={() => onTvHoverChange?.(false)}
    />
    <T.Mesh
